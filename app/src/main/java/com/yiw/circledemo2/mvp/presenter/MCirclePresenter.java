 ...
 */
public class MCirclePresenter extends BasePresenter<MICircleView> {
    private CircleModel mCircleModel;
    List<ListBean> mdatas;
    int loadType;

    public MCirclePresenter() {
    ...
    Handler han = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            Log.d("LD", "datas:" + mdatas.size());
            getView().update2loadData(loadType, mdatas);
        }
    };
            ...
            public void onSuccess(String s) {
                ...
                if (test.getList().size() > 0) {
                    ...
                    for (int i = 0; i < mdatas.size(); i++) {
                        if (mdatas.get(i).getBannerList().size() > 0) {
    ...
    public void addFavort(final int circlePosition) {
        mCircleModel.addFavort(new IDataRequestListener() {
            ...
            public void loadSuccess(Object object) {
                ...
                String id = mdatas.get(circlePosition).getId();
...
//        ap.put("infoId", id);
     ...
     * @Description: 增加评论
     */
    public void addComment(final String content, final MCommentConfig config) {
        if (config == null) {
            return;
        }
        mCircleModel.addComment(new IDataRequestListener() {
            ...
            public void loadSuccess(Object object) {
                ...
                if (config.commentType == MCommentConfig.Type.PUBLIC) {//创建

                    addPP(config, content);
                } else if (config.commentType == MCommentConfig.Type.REPLY) {//回复
    ...
    public void addPP(final MCommentConfig config, final String content) {
        String url = Contantor.record;
        AjaxParams ap = new AjaxParams();
        ap.put("userId", MyApplication.getInstan().getUser().getData().getId() + "");
     ...
     * @param commentConfig
     */
    public void showEditTextBody(MCommentConfig commentConfig) {
